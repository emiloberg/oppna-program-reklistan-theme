...
function showRecDrugsMenu(clickedChapter, clickedEl) {
	...
	jqClicked
		.css('transform','translateY(-' + (clickedRect.top - 53) + 'px)');
...
function doMenuBack(clickedEl) {
	var backTo = $(clickedEl).data('backto');
	if (backTo === 'main-menu') {
		$('#main-menu-placeholder').removeClass('submenu-visible');

		var removeFromEl = $('.has-submenu');



		removeFromEl.removeClass('has-submenu');
		removeFromEl.css('transform', '')
